...
//		}
	...
	public String getGama() {// En getGama dijimos que debe devolver un string.
		String result;
		result = this.gama.toString();
		return result;
	}

	public void setGama(String gama) throws VehiculoException { // setGama puede lanzar una exception.
		if (gama == null) {
			throw new VehiculoException("No se puede asignar una gama nula.");
		}
		try {
			this.gama = GamaEnumerado.valueOf(gama);
		} catch (Exception e) {
			throw new VehiculoException("El valor de la gama introducido no es válido.");
		}
	}
	...
	public double getPrecio() {
		...
		// double result = gama.getPrecio()* (int) fechaEntrada.until(fechaSalida,
		// ChronoUnit.DAYS);
		return result;
	}
